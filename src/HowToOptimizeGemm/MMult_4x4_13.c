                 ...
                 double *c, int ldc)
{
    ...
    { /* Loop over the columns of C, unrolled by 4 */
        ...
        { /* Loop over the rows of C */
            ...
            if (j == 0)
            {
                PackMatrixA(k, &A(i, 0), lda, &packedA[i * k]); // lms:减少pack操作，这样，外层n的循环可以复用pack好的数据
            }
...
{
    int j;
    // lms 将4x4计算用到的的左矩阵A[i...i+3, p]按列pack到[4,k]这样ld为4的矩阵，保证
    //  innerkernel里在K的循环上，内存连续，cache miss率小
